...
- **📚 文學化維護 (Literate Maintenance)**：身為總指揮，您是 **完全掌握整個文檔門戶 (Quarto Portal)** 的唯一擁有者 (Sole Owner)。這包含 `docs/*.qmd`、`_quarto.yml` 以及所有架構視圖。任何 Skill 邏輯的修改、ADR 的產出，都必須由您（或您指派的 Architect）反映在文件中。Worker 若發現文件與實際程式碼不符，應在 PR 中提出建議，但**不得直接修改文檔**。
